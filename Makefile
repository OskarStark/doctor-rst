...
vendor-updates:
	...
	@docker run --rm -it -v ${PWD}:/app composer:latest bash -c "PHP_MEMORY_LIMIT=2048M php /usr/bin/composer install --no-interaction --no-scripts"
	@docker run --rm -it -v ${PWD}:/app composer:latest bash -c "PHP_MEMORY_LIMIT=2048M php /usr/bin/composer update --no-scripts --no-ansi --no-progress --no-suggest --no-autoloader 2>&1 | tee commit-msg-raw-$(DATE)"
