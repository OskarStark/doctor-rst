...
vendor-updates:
	...
	@docker run --rm -it -v ${PWD}:/usr/src/app exozet/php-fpm:7.3 bash -c "PHP_MEMORY_LIMIT=2048M php /usr/local/bin/composer install --no-interaction --no-scripts"
	@docker run --rm -it -v ${PWD}:/usr/src/app exozet/php-fpm:7.3 bash -c "PHP_MEMORY_LIMIT=2048M php /usr/local/bin/composer update --no-scripts --no-ansi --no-progress --no-suggest --no-autoloader 2>&1 | tee commit-msg-raw-$(DATE)"
